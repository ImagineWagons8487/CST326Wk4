...
public class LevelParser : MonoBehaviour
{
    ...
    void Update()
    {
        if (Input.GetKeyDown(KeyCode.R))
            ReloadLevel();
        // Vector2 currentOffset = newObj.GetComponent<Renderer>().material.mainTextureOffset;
        // newObj.GetComponent<Renderer>().material.mainTextureOffset =
        //     new Vector2(currentOffset.x, currentOffset.y + .2f);
    }
    ...
    void LoadLevel()
    {
        ...
        while (levelRows.Count > 0)
        {
            ...
            for (int col = 0; col < letters.Length; ++col)
            {
                ...
                Vector3 pos = new Vector3(col+.5f, row+.5f, 0f);
                GameObject newObj;
                switch (letters[col])
                {
                    case 'x':
                    ...
                    case 's':
                        ...
                        break;
                    case 'w':
                        newObj = Instantiate(lavaPrefab, environmentRoot);
                        newObj.transform.position = pos;
                        break;
                    case 'g':
                        newObj = Instantiate(goalPrefab, environmentRoot);
                        newObj.transform.position = pos;
                        break;
                }
